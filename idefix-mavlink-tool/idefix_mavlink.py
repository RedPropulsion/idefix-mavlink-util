...
The sender uses the eight project-specific MAV_CMD values inside the standard
...
def parse_mode(actuator: str, value: str) -> IntEnum:
    """Parse friendly names such as ``wiggle`` or ``SERVO_WIGGLE``."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Tool MAVLink di Idefix per comandare o simulare ObeliCS"
    )
    subparsers = parser.add_subparsers(dest="action", required=True)

    receiver = subparsers.add_parser(
        "listen", help="simula ObeliCS: riceve e conferma i comandi"
    )
    ...
    sender = subparsers.add_parser("send", help="invia un singolo comando da Idefix")
    ...
    shell = subparsers.add_parser(
        "interactive", help="apre una console interattiva su Idefix"
    )
